...
//  WtlFileTreeCtrl.cpp		14.06.2007	Added 'Synchronize with current file' context menu option
//  WtlFileTreeCtrl.cpp		26.06.2007	Added 'Open Folder' context menu option
//										Added 'Open command line here' context menu option
//
//  For version 1.6
//
//  WtlFileTreeCtrl.cpp		08.10.2008	Fixed message for SearchInFiles plugin when invoked 'Search from here' menu
//
//
//  For version 2.0 UNICODE
//
//	UNICODE update made by Jugoslav Dujic
//
//  utils.h					21.03.2009	Changed UTL_memmove to use wmemmove
...
const TCHAR	PLUGIN_NAME[] = _T("Light Explorer");
const int	nbFunc = 3;
const TCHAR	localConfFile[]	= _T("doLocalConf.xml");
...
BOOL APIENTRY DllMain(HANDLE hModule, DWORD  reasonForCall, LPVOID lpReserved)
{
    ...
    switch (reasonForCall)
    {
		case DLL_PROCESS_ATTACH:
		{
			...
			_tcscpy(funcItem[DOCKABLE_LIGHTEXPLORER]._itemName, _T("Light Explorer"));
			_tcscpy(funcItem[1]._itemName, _T("-----------"));
			_tcscpy(funcItem[2]._itemName, _T("H&elp ..."));
			...
			if (isLocal) 
			{
				_tcscpy(_lightExplorerDlg.m_iniFilePath, nppPath);
				PathAppend(_lightExplorerDlg.m_iniFilePath, _T("plugins\\lightExplorer.ini"));
			}
			else 
			{
				ITEMIDLIST *pidl;
				...
				SHGetPathFromIDList(pidl, _lightExplorerDlg.m_iniFilePath);
	  			...
	  			PathAppend(_lightExplorerDlg.m_iniFilePath, _T("Notepad++\\plugins\\lightExplorer.ini"));
			}
