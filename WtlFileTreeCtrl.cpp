...
BOOL CWtlFileTreeCtrl::SubclassWindow(lightExplorerDlg* ownerDockingDlgInterface, HWND hWnd, LPTSTR iniFilePath) {
	...
	if(bRet) {
		...
		m_useSystemIcons = confIni.LoadInt(_T("LightExplorer"), _T("useSystemIcons"), 1) ? true : false;
...
void CWtlFileTreeCtrl::OnViewRefresh() {
	...
	tstring sItem  = GetItemTag(hSelItem);
...
void CWtlFileTreeCtrl::InsertRoots() {
	try {
		...
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::InsertRoots."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
}

HTREEITEM CWtlFileTreeCtrl::AddSystemRoot(int nFolder, LPCTSTR defaultName) {
	try {
		// Add a 'system' root
		...
		if (SUCCEEDED(SHGetSpecialFolderLocation(NULL, nFolder, &lpMCPidl))) {
			...
			tvis.item.pszText			= (LPTSTR)itemName.GetSafe();
		...
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::AddSystemRoot."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
	return NULL;
}


int CWtlFileTreeCtrl::CompareByFilenameNoCase(tstring element1, tstring element2) {
...
LPCTSTR CWtlFileTreeCtrl::GetSelectedPath() {
  return GetItemTag(GetSelectedItem());
}

LPCTSTR CWtlFileTreeCtrl::GetItemDisplayString(HTREEITEM hItem) {
	...
	tvi.hItem	= hItem;
	if(!GetItem(&tvi)) {
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::GetItemTag"), _T("WtlFileTreeCtrl.cpp"), __LINE__);
		return NULL;
	}
	else {
...
bool CWtlFileTreeCtrl::SetItemDisplayString(HTREEITEM hItem, LPCTSTR newDisplayString) {
	...
	tvi.hItem	= hItem;
	if(!GetItem(&tvi)) {
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::SetItemDisplayString"), _T("WtlFileTreeCtrl.cpp"), __LINE__);
		return false;
	}
	else {
		CCustomItemInfo* pCii = (CCustomItemInfo*)tvi.lParam;
...
LPCTSTR CWtlFileTreeCtrl::GetItemTag(HTREEITEM hItem) {
	if (hItem == NULL) return _T("");
...
bool CWtlFileTreeCtrl::SetItemTag(HTREEITEM hItem, LPCTSTR newTag) {
...
NETRESOURCE* CWtlFileTreeCtrl::GetNetworkResource(HTREEITEM hItem) {
	...
	tvi.hItem	= hItem;
	if(!GetItem(&tvi)) {
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::GetNetworkResource"), _T("WtlFileTreeCtrl.cpp"), __LINE__);
		return NULL;
	}
	else {
		CCustomItemInfo* pCii = (CCustomItemInfo*)tvi.lParam;
...
void CWtlFileTreeCtrl::SetRootFolder(const tstring sPath) {
...
HTREEITEM CWtlFileTreeCtrl::InsertTreeItem(LPCTSTR sFile, LPCTSTR sPath, HTREEITEM hParent, bool isFolder, LPCTSTR desc) {
	// Retreive the icon indexes for the specified file/folder
	CUTL_BUFFER fullPath(sPath);

	if (isFolder && fullPath[fullPath.Len() - 1] != _T('\\')) fullPath += _T("\\");
	...
	displayString = (sFile != _T("")) ? sFile : ((desc != NULL) ? desc : sPath);
	...
	tvis.hInsertAfter			= TVI_LAST;
	tvis.item.mask				= TVIF_CHILDREN | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_TEXT | TVIF_PARAM;
	tvis.item.pszText			= (LPTSTR)displayString.GetSafe();
...
int CWtlFileTreeCtrl::GetOverlayIcon(LPCTSTR sPath, bool isFolder) {
	SHFILEINFO		sfi	= {0};
...
HTREEITEM CWtlFileTreeCtrl::InsertTreeNetworkItem(HTREEITEM hParent, LPCTSTR sFQPath, CCustomItemInfo* pCii) {
	// Retreive the icon indexes for the specified file/folder
	...
	tvis.hInsertAfter			= TVI_LAST;
	tvis.item.mask				= TVIF_CHILDREN | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_TEXT | TVIF_PARAM;
	tvis.item.pszText			= (LPTSTR)pCii->getDisplayString();
...
void CWtlFileTreeCtrl::GetDriveLabel(const CUTL_BUFFER& bufDrive, CUTL_BUFFER& bufDriveLabel) {
	...
	if (bufDriveLabel[bufDriveLabel.Len() - 1] == ')' && bufDriveLabel[bufDriveLabel.Len() - 4] == '(') {
		...
		if (temp2[temp2.Len() - 1] == ')' && temp2.Find(' ', found)) {
			...
			temp4[temp4.Len() - 1] = _T('\0'); 

			temp5.Sf(_T("%s\\%s"), temp4.GetSafe(), temp3.GetSafe());
			temp2 = temp5;
		}

		temp.Sf(_T("%s %s"), &bufDriveLabel[bufDriveLabel.Len() - 4], temp2.GetSafe());
		bufDriveLabel = temp;
	}
	...
	for(int i = 0; i < 32; i++ ) {
		if( dwDrives & dwMask ) {
			...
			if( sDrive == _T("A:\\") || sDrive == _T("B:\\") ){}
				...
				InsertTreeItem(_T(""), sDrive.c_str(), hParent, true, volumenName.GetSafe());
			}
		}
		dwMask <<= 1;
	}
...
void CWtlFileTreeCtrl::DisplayPath(LPCTSTR folder, HTREEITEM parentItem) {
	CWaitCursor c;

	// Folders and files under buf
	CUTL_BUFFER buf(folder);
	if (buf[buf.Len() - 1] != _T('\\')) buf += _T("\\");
	...
	iterator.SetNameExtension(_T("*.*"));
	folderIt.SetNameExtension(_T("*.*"));
	...
	while(bIterating) {
		folderName = (LPCTSTR)folderIt;

		if (folderName[folderName.Len() - 1] == _T('\\')) folderName[folderName.Len() - 1] = _T('\0');
		folderName.Reverse();
		folderName.Find(_T("\\"), found);
		folderName[found] = _T('\0');
		folderName.Reverse();

		listElement.name		= folderName.GetSafe();
		listElement.iterator	= (LPCTSTR)folderIt;
	...
	for (size_t i = 0; i < vFolderList.size(); i++) 
		InsertTreeItem((LPTSTR)vFolderList[i].name.GetSafe(), (LPCTSTR)(LPTSTR)vFolderList[i].iterator, parentItem, true);
	...
	while(bIterating) {
		...
		listElement.iterator	= (LPCTSTR)iterator;
	...
	for (size_t j = 0; j < vFilesList.size(); j++) 
		InsertTreeItem((LPTSTR)vFilesList[j].name.GetSafe(), (LPCTSTR)(LPTSTR)vFilesList[j].iterator, parentItem, false);
}
...
void CWtlFileTreeCtrl::QuickSortItems(std::vector<tItemList>* vList, INT d, INT h) {
	...
	do {
		while (_tcsicmp((*vList)[j].name.GetSafe(), str.c_str()) < 0) j++;
		while (_tcsicmp((*vList)[i].name.GetSafe(), str.c_str()) > 0) i--;
...
HTREEITEM CWtlFileTreeCtrl::FindSibling(HTREEITEM hParent, const tstring sItem) {
	HTREEITEM hChild = GetChildItem(hParent);
	TCHAR sFound[ _MAX_PATH ];
	while( hChild )
	{
...
HTREEITEM CWtlFileTreeCtrl::SetSelectedPath(const tstring sPath, BOOL bExpanded) {
...
BOOL CWtlFileTreeCtrl::onDeleteItem(int idCtrl, LPNMHDR pnmh, BOOL& bHandled) {
	try {
		...
		systemMessage(_T("Error at CWtlFileTreeCtrl::onDeleteItem."));
	}
	return 0;
}
...
BOOL CWtlFileTreeCtrl::OnItemClick(int idCtrl, LPNMHDR pnmh, BOOL& bHandled) {
	try {
		...
		systemMessage(_T("Error at CWtlFileTreeCtrl::OnItemClick."));
	}
	return 0;
}
...
BOOL CWtlFileTreeCtrl::OnSelChanged(int idCtrl, LPNMHDR pnmh, BOOL& bHandled) {
	try {
		...
		if (cii != NULL) {
			...
			if (fullPath[0] == '\\' && fullPath[1] == _T('\\')) {
				CUTL_BUFFER tempBuf(cii->getTag());
				UINT lastSlash;

				tempBuf.ReverseFind(_T('\\'), lastSlash);
      ...
      int itemType = cii->getType();

      if ((itemType==CCustomItemInfo::FOLDER || itemType==CCustomItemInfo::FILE) && folderName[folderName.Len() - 1] != _T('\\')) 
        folderName += _T("\\");
		...
		systemMessage(_T("Error at CWtlFileTreeCtrl::OnSelChanged."));
	}
	return 0;
}
...
void CWtlFileTreeCtrl::SetObjects(tstring strObject) {
...
void CWtlFileTreeCtrl::SetObjects(std::vector<tstring> strArray) {
...
#else
  _tcscpy(olePath, strArray[0].c_str());
#endif
...
#else
      _tcscpy(olePath, strArray[i].c_str());
#endif
			psfDesktop->ParseDisplayName (NULL, 0, olePath, NULL, &pidl, NULL);
			free (olePath);
...
BOOL CWtlFileTreeCtrl::OnRClickItem(int idCtrl, LPNMHDR pnmh, BOOL& bHandled, BOOL byKeyboard) {
	try {
		...
		bool		initialLoadOnStartup = confIni.LoadInt(_T("startContext"), _T("loadOnStartup"), 1) ? true : false;
		...
		if (hHitItem == NULL || (!(TVHT_ONITEMICON & uFlags) && !(TVHT_ONITEMLABEL & uFlags))) {
			...
			mii.dwTypeData	= _T("Synchronize tree with current document");
			...
			mii.dwTypeData	= _T("Open command line here");
			mii.cch			= UTL_strlen(mii.dwTypeData);
			mii.fState		= MFS_DISABLED;
			...
			mii.dwTypeData	= _T("Load tree state on startup");
			mii.cch			= UTL_strlen(mii.dwTypeData);
			...
			mii.dwTypeData	= _T("Use system icons");
			mii.cch			= UTL_strlen(mii.dwTypeData);
			...
			switch (resp) 
			{
				...
				case LOAD_LAST_SESSION:
					confIni.Write(_T("startContext"), _T("loadOnStartup"), initialLoadOnStartup ? _T("0") : _T("1"));
					break;
		...
		if (itemType == CCustomItemInfo::FILE || itemType == CCustomItemInfo::FOLDER) {
			...
			mii.dwTypeData	= _T("Add to favorites");
			mii.cch			= UTL_strlen(mii.dwTypeData);
			...
			// Don't offer to search on root disks
			if (itemTag[itemTag.Len() - 2] != ':') {
				// Search from here
				mii.wID			= SEARCH_FROM_HERE;
				mii.dwTypeData	= _T("Search from here");
				mii.cch			= UTL_strlen(mii.dwTypeData);
				mii.fState		= MFS_ENABLED;
			...
			if (itemTag[0] != _T('\\') && itemTag[1] != _T('\\') && itemTag[itemTag.Len() - 2] != _T(':')) {
				// Custom Delete
			...
			mii.dwTypeData	= _T("Remove from favorites");
			mii.cch			= UTL_strlen(mii.dwTypeData);
			mii.fState		= MFS_ENABLED;
			...
			mii.dwTypeData	= _T("Edit favorite folder name");
			mii.cch			= UTL_strlen(mii.dwTypeData);
			mii.fState		= MFS_ENABLED;
		...
		if (m_pidlArray != NULL && hStandardMenu != NULL) {
			...
			::AppendMenu(hPopupMenu, MF_SEPARATOR, 0, 0);
			::InsertMenu(hPopupMenu, -1, MF_BYPOSITION | MF_STRING | MF_POPUP, (UINT_PTR)hStandardMenu, _T("Standard Menu"));
		}
		...
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::OnRClickItem."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
	return 0;
}
...
void CWtlFileTreeCtrl::DoOpenCommandLine() {
	try {
		...
		parameters.Sf(_T("/E:ON /k pushd \"%s\""), (LPCTSTR)currFullPath); 
		...
		if (!ShellExecuteEx(&si)) 
			systemMessage(bufTemp.Sf(_T("Error opening cmd on folder '%s'."), driveDir.GetSafe()));
	}
	catch (...) {
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::DoOpenCommandLine."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
}
...
void CWtlFileTreeCtrl::DoOpenFolder() {
	try {
		...
		if (!ShellExecuteEx(&si)) 
			systemMessage(bufTemp.Sf(_T("Error executing folder'%s'."), driveDir.GetSafe()));
	}
	catch (...) {
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::DoOpenFolder."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
}
...
void CWtlFileTreeCtrl::DoCustomDelete() {
	try {
		...
		msg.Sf(_T("Delete %s '%s'"), itemType == CCustomItemInfo::FOLDER ? _T("folder") : _T("file"), (LPCTSTR)current);
		if (IDNO == ::MessageBox(m_hWnd, msg.GetSafe(), _T("lightExplorer"), MB_YESNO | MB_ICONQUESTION)) return;

		if (itemType == CCustomItemInfo::FOLDER) {
			if (current.RemoveDirectory())
				DeleteItem(hHitItem);
			else 
				systemMessage(msg.Sf(_T("Could not delete folder '%s'"), (LPCTSTR)current));
		}
		else if (itemType == CCustomItemInfo::FILE) {
			if (current.Delete(TRUE))
				DeleteItem(hHitItem);
			else {
				systemMessage(msg.Sf(_T("Could not delete file '%s'"), (LPCTSTR)current));
			}
		}
	}
	catch (...) {
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::DoCustomDelete."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
}
...
BOOL CALLBACK EditDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	try {
			...
			case WM_INITDIALOG:
				{
					...
					if (ownerDlg->GetActionState() == CWtlFileTreeCtrl::CUSTOM_RENAME_STATE) {
						...
						if (ownerDlg->GetItemType(ownerDlg->GetSelectedItem()) == CCustomItemInfo::FILE) {
							...
							message.Sf(_T("Rename file '%s' as "), itemName.GetSafe());
						}
						else {
							fullPath.GetDirectory(itemName);

							if (itemName.ReverseFind(_T('\\'), found)) {
								temp = &itemName[found + 1];
								itemName = temp;
							}

							message.Sf(_T("Rename folder '%s' as "), itemName.GetSafe());
						}
						...
						::SetWindowText(::GetDlgItem(hDlg, IDC_EDIT_MESSAGE), _T("Create a new folder:"));
			...
			case WM_COMMAND:
				{
					if (LOWORD(wParam) == IDOK) {
						...
						if (!newName.Len()) {
							::MessageBox(hDlg, _T("New name cannot be empty ..."), _T("Light Explorer"), MB_OK | MB_ICONSTOP);
							::SetFocus(::GetDlgItem(hDlg, IDC_EDIT_MESSAGE));
							break;
						}
						...
						if (ownerDlg->GetActionState() == CWtlFileTreeCtrl::CUSTOM_RENAME_STATE) {
							...
							if (ownerDlg->GetItemType(ownerDlg->GetSelectedItem()) == CCustomItemInfo::FILE) {
								...
								if (fullPath.Rename(newPath)) {
									...
									ownerDlg->SetItemTag(ownerDlg->GetSelectedItem(), (LPCTSTR)newPath);
									ownerDlg->SetItemDisplayString(ownerDlg->GetSelectedItem(), newName.GetSafe());
								}
								else {
									systemMessage(message.Sf(_T("Unable to rename '%s' as '%s':"), (LPCTSTR)fullPath, (LPCTSTR)newPath));
									::SetFocus(::GetDlgItem(hDlg, IDC_EDIT_MESSAGE));
									::SendMessage(::GetDlgItem(hDlg, IDC_EDIT), CB_SETCURSEL, 0, 0L);
									break;
								}
								...
								if (itemName.ReverseFind(_T('\\'), found) && found) {
								...
								message.Sf(_T("Rename folder '%s' as '%s'"), (LPCTSTR)fullPath, (LPCTSTR)newPath);
								...
								if (fullPath.Rename(newPath)) {
									...
									systemMessage(message.Sf(_T("Unable to rename folder '%s' as '%s':"), (LPCTSTR)fullPath, (LPCTSTR)newPath));
									::SetFocus(::GetDlgItem(hDlg, IDC_EDIT_MESSAGE));
									::SendMessage(::GetDlgItem(hDlg, IDC_EDIT), CB_SETCURSEL, 0, 0L);
									break;
								}
							...
							if (fullPath.DirectoryExists()) {
								...
								if (fullPath.CreateDirectory()) {
									...
									HTREEITEM newItem = ownerDlg->InsertTreeItem(newName.GetSafe(), (LPCTSTR)fullPath, ownerDlg->GetSelectedItem(), true);
									...
									systemMessage(message.Sf(_T("Unable to create folder '%s':"), (LPCTSTR)fullPath));
									::SetFocus(::GetDlgItem(hDlg, IDC_EDIT_MESSAGE));
									::SendMessage(::GetDlgItem(hDlg, IDC_EDIT), CB_SETCURSEL, 0, 0L);
									break;
								}
		...
		systemMessageEx(_T("Error at EditDlgProc"), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
	return FALSE;
}
...
void CWtlFileTreeCtrl::SynchronizeTree() {
	...
	if (currDocBuffer[0] == _T('\\'))
		...
		while(currDocBuffer.Find(_T("\\"), found, start)) {
		...
		confIni.Write(_T("startContext"), _T("currItem"), result.GetSafe());
...
void CWtlFileTreeCtrl::DoCustomNewFolder() {
	try {
		...
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::DoCustomNewFolder."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
}
...
void CWtlFileTreeCtrl::DoCustomRename() {
	try {
		...
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::DoCustomRename."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
}
...
void CWtlFileTreeCtrl::DoChangeUseSystemIcons() {
	try {
		...
		confIni.Write(_T("LightExplorer"), _T("useSystemIcons"), m_useSystemIcons ? _T("0") : _T("1"));
		// Save current 
		SaveState();
		// Read current config
		int loadOnStartUp = confIni.LoadInt(_T("startContext"), _T("loadOnStartup"), 1);
		// Change it
		confIni.Write(_T("startContext"), _T("loadOnStartup"), _T("1"));
		// Delete everything
		this->DeleteAllItems();
		// Rebuild all
		PostMessage(WM_POPULATE_TREE);
		// Put back original state
		confIni.Write(_T("startContext"), _T("loadOnStartup"), loadOnStartUp ? _T("1") : _T("0"));
	}
	catch (...) {
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::DoChangeUseSystemIcons."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
}
...
void CWtlFileTreeCtrl::ExecuteFile() {
	try {
		...
		if (!ShellExecuteEx(&si)) {
			...
			systemMessage(bufTemp.Sf(_T("Error executing file'%s'."), GetItemTag(hSelItem)));
		}
	}
	catch (...) {
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::OpenFile."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
}

HWND CWtlFileTreeCtrl::LookForSearchInFilesPlugin(HWND hSearchWnd) {
	HWND hTemp;

	try {
		CUTL_BUFFER windowText(256);

		while (hSearchWnd) {
			hTemp = ::GetWindow(hSearchWnd, GW_CHILD);

			if (hTemp) {
				if (hTemp = LookForSearchInFilesPlugin(hTemp)) return hTemp;
			}

			::GetWindowText(hSearchWnd, windowText.data, 255);
			if (windowText == "Search in Files")
				return hSearchWnd; // SUCCESS!
			
			hSearchWnd = ::GetWindow(hSearchWnd, GW_HWNDNEXT);
		}
	}
	catch (...) {
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::LookForSearchInFilesPlugin."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
	return hSearchWnd;
}
...
void CWtlFileTreeCtrl::SearchFromHere() {
	try {
		...
		// We search for 'Search in Files' window
		HWND hProp = LookForSearchInFilesPlugin(::GetWindow(m_nppHandle, GW_CHILD));

		CUTL_PATH fullPath(GetItemTag(hSelItem));
		CUTL_BUFFER driveDir, tempBuf, extension;

		if (fullPath[0] == _T('\\') && fullPath[1] == _T('\\')) {
		...
		if (tempBuf == _T("")) 
			...
			extension.Sf(_T("*.%s"), tempBuf.GetSafe());

		if (hProp != NULL)  
			::SendMessage(hProp, WM_PG_LAUNCH_SEARCHINFILESDLG, (WPARAM)extension.GetSafe(), (LPARAM)driveDir.GetSafe());
		else 
			::SendMessage(m_nppHandle, WM_LAUNCH_FINDINFILESDLG, (WPARAM)driveDir.GetSafe(), (LPARAM)extension.GetSafe());
	}
	catch (...) {
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::SearchFromHere."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
}
...
void CWtlFileTreeCtrl::AddEditFavoriteFolderName() {
	try {
		...
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::AddEditFavoriteFolderName."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
}
...
void CWtlFileTreeCtrl::ShowProperties() {
	try {
		...
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::ShowProperties."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
}
...
bool CWtlFileTreeCtrl::EnumNetwork(HTREEITEM hParent) {
	...
	try {
		...
		if (dwResult != NO_ERROR) {
			systemMessageEx(_T("CWtlFileTreeCtrl::enumNetwork, Cannot enumerate network drives."), _T("WtlFileTreeCtrl.cpp"), __LINE__);
			return bGotChildren;
		} 
		...
		if (bSuccess) {
			...
			systemMessageEx(err.Sf(_T("CWtlFileTreeCtrl::EnumNetwork: cannot complete network drive enumeration, Error:%d."), dwResult), _T("WtlFileTreeCtrl.cpp"), __LINE__);
		}
		...
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::enumNetwork"), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
	return bGotChildren;
}
...
void CWtlFileTreeCtrl::SaveFavorites() {
	...
	while(hFavorite) {
		...
		confIni.Write(_T("Favorites"), pCii->getDisplayString(), pCii->getTag());
...
void CWtlFileTreeCtrl::LoadFavorites() {
	try {
		...
		confIni.LoadEntries(_T("Favorites"));

		while (!!(entry = confIni.GetEntry(i++))) {
			CUTL_BUFFER itemTag = confRead.LoadStr(_T("Favorites"), entry);
			...
			tvis.hInsertAfter			= TVI_LAST;
			tvis.item.mask				= TVIF_CHILDREN | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_TEXT | TVIF_PARAM;
			tvis.item.pszText			= (LPTSTR)UTL_Null(entry);
		...
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::LoadFavorites"), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
}
...
void CWtlFileTreeCtrl::LoadState() {
	try {
		...
		if (!confIni.LoadInt(_T("startContext"), _T("loadOnStartup"), 1)) return;

		startContext.Sf(_T("#%s;"), confIni.LoadStr(_T("startContext"), _T("currItem"), _T("")));

		CUTL_PARSE startContextParse(startContext.GetSafe(), NULL, _T('#'));
		...
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::SaveState"), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
}
...
void CWtlFileTreeCtrl::SaveState() {
	try {
		...
		confIni.Delete(_T("startContext"), _T("currItem"));
		...
		while (currParent) {
			...
			pathToSave = tempBuf2.Sf(_T("%s%s%s"), 
										tempBuf.GetSafe(), 
										pathToSave.Len() ? _T(",") : _T(""), 
										pathToSave.GetSafe());
		...
		confIni.Write(_T("startContext"), _T("currItem"), pathToSave.GetSafe());
	}
	catch (...) {
		systemMessageEx(_T("Error at CWtlFileTreeCtrl::SaveState"), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
...
BOOL CALLBACK FavoritesFolderNameDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	try {
			...
			case WM_INITDIALOG:
				{
					...
					::SetWindowText(::GetDlgItem(hDlg, IDC_EDIT_NAME), (LPTSTR)pCurrentCii->getDisplayString());
			...
			case WM_COMMAND:
				{	
					...
					if (LOWORD(wParam) == IDOK) {
						...
						::GetWindowText(::GetDlgItem(hDlg, IDC_EDIT_NAME), (LPTSTR)favoriteFolderName.GetSafe(), 254);
		...
		systemMessageEx(_T("Error en SearchInputDlg::SearchInFilesInputDlgProc"), _T("WtlFileTreeCtrl.cpp"), __LINE__);
	}
	return FALSE;
}
