...
CUTL_BUFFER::CUTL_BUFFER(LPCTSTR str) {
   data= NULL; length=0;
   Copy(str);
}

CUTL_BUFFER::CUTL_BUFFER(UINT size) {
   if (size > 0) length = _CRVERIFY(data = (LPTSTR) UTL_alloc(size, sizeof(TCHAR))) ? size : 0;
   else {
      data = NULL;
      length = 0;
   }
...
BOOL CUTL_BUFFER::Realloc(UINT size) {
   ...
   length = 0;
   if (!size) return TRUE;
   if (!(data = (LPTSTR) UTL_alloc(size, sizeof(TCHAR)))) return false;    
   length = size;
   return TRUE;
}
...
LPTSTR CUTL_BUFFER::NCopy(LPCTSTR string2, size_t count) {
...
LPTSTR CUTL_BUFFER::Copy(LPCTSTR string2) {
...
LPTSTR CUTL_BUFFER::Cat(LPCTSTR string2) {
   _CRCHECK(!data || data != string2);
   if (!data) {
      _CRVERIFY(Realloc(UTL_strlen(string2) +1));
      ...
      LPTSTR p = (LPTSTR) UTL_alloc(length = (Len() + UTL_strlen(string2) + 1), sizeof(TCHAR));
      if (!p) return data;    
      UTL_strcpy(p, data);
...
LPTSTR CUTL_BUFFER::InsStr(LPCTSTR str, UINT numChars, UINT pos, BOOL forceUseNumChars) {
   _CRVERIFY(pos <= Len());
   ...
   if (length < newLen) {
      ...
      LPTSTR p = (LPTSTR) UTL_alloc(length = newLen, sizeof(TCHAR));
      if (!p) return data;    
      UTL_strcpy(p, data);
      UTL_free(data);
      data = p;
...
LPTSTR CUTL_BUFFER::InsCar(TCHAR ch, UINT pos) {
   _CRVERIFY(pos <= Len());
   if (!data) _CRVERIFY(Realloc(2));
   if (length == Len() + 1) {
      // se expande el buffer + 10 para permitir algunas llamadas reiteradas sin cambiar de buffer
      LPTSTR p = (LPTSTR) UTL_alloc(length += 10, sizeof(TCHAR));
      if (!p) return data;    
      UTL_strcpy(p, data);
      UTL_free(data);
      data = p;
   ...
   UTL_memmove(data + pos + 1, data + pos, UTL_strlen(data) - pos + 1);
   data[pos] = ch;
   return data;
}

UINT CUTL_BUFFER::RepCar(TCHAR replaceCh, TCHAR withCh, UINT start) {
   //_CRCHECK(start < Len());
   if (!data) return 0;
...
CUTL_BUFFER &CUTL_BUFFER::Sf(LPCTSTR fmt, ... ) {
...
CUTL_BUFFER &CUTL_BUFFER::SfVa(LPCTSTR fmt, va_list args) {
   _CRCHECK(!data || data != fmt);
   BOOL enough = FALSE;
   static UINT step = 512;
...
long   CUTL_BUFFER::AToL() { return _ttol(UTL_Null(data)); }
double CUTL_BUFFER::AToF() { return _tstof(UTL_Null(data)); }
int    CUTL_BUFFER::AToI() { return _ttoi(UTL_Null(data)); }

LPCTSTR CUTL_BUFFER::IToA(int value, int radix) {
   if (length <= 40) Realloc(40);
   return _itot(value, data,radix);
}

LPCTSTR CUTL_BUFFER::LToA(long value, int radix) {
   if (length <= 40) Realloc(40);
   return _ltot(value, data,radix);
}

LPCTSTR CUTL_BUFFER::ULToA(unsigned long value, int radix) {
   if (length <= 40) Realloc(40);
   return _ultot(value, data,radix);
}
...
BOOL CUTL_BUFFER::Find(LPCTSTR string, UINT &found, UINT start) const {
   if (!data || start >= Len()) return FALSE;
   _CRCHECK(string);
   LPCTSTR p = UTL_strstr(data + start, string);
   if (p) {
      found = (UINT) (p - data);
      return TRUE;
   }
...
BOOL CUTL_BUFFER::Find(const TCHAR ch, UINT &found, UINT start) const {
   if (!data || start >= Len()) return FALSE;
   LPCTSTR p = UTL_strchr(data + start, ch);
   if (p) {
      found = (UINT) (p - data);
      return TRUE;
   }
...
BOOL CUTL_BUFFER::ReverseFind(const TCHAR ch, UINT &found) const {
   if (!data) return FALSE;
   LPCTSTR p = UTL_strrchr(data, ch);
   if (p) {
      found = (UINT) (p - data);
      return TRUE;
   }
...
BOOL CUTL_BUFFER::FindOneOf(LPCTSTR set, UINT &found) const {
   if (!data) return FALSE;
...
TCHAR &CUTL_BUFFER::operator[](UINT index) const {
   static TCHAR x = _T('\0');
...
TCHAR &CUTL_BUFFER::operator[](int index) const {
   static TCHAR x = _T('\0');
...
CUTL_BUFFER &CUTL_BUFFER::operator=(LPCTSTR str) {
...
//--- Constructores ---------------------------------------------------------------------------------------------------
CUT2_INI::CUT2_INI(LPCTSTR fileName) { 
   m_int  = 0;
   m_long = 0L;
   m_file = fileName;
}
...
int CUT2_INI::LoadInt(LPCTSTR section, LPCTSTR entry, int def) {
...
long CUT2_INI::LoadLong(LPCTSTR section, LPCTSTR entry, long def) {
   TCHAR convert[26];

   if (!_CRVERIFY(m_file.data)) return 0; //Falta definir el fichero con SetFileName
   _CRCHECK(section && entry);
   if (!::GetPrivateProfileStringW(section, entry, _T(""), convert, 25, m_file)) return def;
   return m_long = UTL_atol(convert);
}

LPCTSTR CUT2_INI::LoadStr(LPCTSTR section, LPCTSTR entry, LPCTSTR def) {
   int size;
   CUTL_BUFFER buf(10*1024);                          

   if (!_CRVERIFY(m_file.data)) return 0; //Falta definir el fichero con SetFileName
   _CRCHECK(section && entry);
   size = ::GetPrivateProfileStringW(section, entry, UTL_Null(def), LPTSTR(buf), 10*1024, m_file);
   CUTL_BUFFER::operator=(buf);    
   return (size) ? data : NULL;
}

BOOL CUT2_INI::Write(LPCTSTR section, LPCTSTR entry, LPCTSTR str) const {
...
BOOL CUT2_INI::Write(LPCTSTR section, LPCTSTR entry, int value) const {
   TCHAR convert[10];
...
BOOL CUT2_INI::Write(LPCTSTR section, LPCTSTR entry, long value) const {
   TCHAR convert[26];
...
BOOL CUT2_INI::Delete(LPCTSTR section, LPCTSTR entry) const {
   if (!_CRVERIFY(m_file.data)) return FALSE; //Falta definir el fichero con SetFileName
   _CRCHECK(section);
...
LPCTSTR CUT2_INI::LoadEntries(LPCTSTR section) {
   int size;
   CUTL_BUFFER buf(64*1024);
 
   if (!_CRVERIFY(m_file.data)) return 0; // Falta definir el fichero con SetFileName
   _CRCHECK(section);
   size = ::GetPrivateProfileStringW(section, NULL, _T(""), LPTSTR(buf), 64*1024, m_file);
   Realloc(size+2);
   UTL_memcpy(data, LPTSTR(buf), size+2);      
   return (size) ? data : NULL;
}

LPCTSTR CUT2_INI::LoadSections() {
   int size;
   CUTL_BUFFER buf(10*1024);

   if (!_CRVERIFY(m_file.data)) return 0; //Falta definir el fichero con SetFileName
   size = ::GetPrivateProfileStringW(NULL, NULL, _T(""), LPTSTR(buf), 10*1024, m_file);
   Realloc(size+2);
   UTL_memcpy(data, LPTSTR(buf), size+2);      
   return (size) ? data : NULL;
}

LPCTSTR CUT2_INI::GetEntry(UINT pos) const {
   int i = 0, j;
   
   for (i=0; pos && (j = UTL_strlen((LPTSTR) data + i)); i += j+1, pos--); 
   return UTL_strlen((LPTSTR) data + i) ? (LPTSTR) data + i : NULL;
}
...
CUTL_PATH::CUTL_PATH(LPCTSTR path) {
   xInit();
   m_path = path;
}
...
void CUTL_PATH::Set(LPCTSTR path) {
   m_path = path;
}

void CUTL_PATH::SetDrive(TCHAR driveLeter) {
   CUTL_BUFFER drive(2);
...
void CUTL_PATH::SetDriveDirectory(LPCTSTR pDriveDirectory) {
...
void CUTL_PATH::SetDirectory(LPCTSTR pDirectory, BOOL ensureAbsolute) {
   CUTL_BUFFER drive;
...
void CUTL_PATH::SetName(LPCTSTR pName) {
   CUTL_BUFFER drive;
   CUTL_BUFFER directory;
   CUTL_BUFFER extension;
...
void CUTL_PATH::SetNameExtension(LPCTSTR pNameExtension) {
   CUTL_BUFFER drive;
   CUTL_BUFFER directory;
...
void CUTL_PATH::SetExtension(LPCTSTR pExtension) {
   CUTL_BUFFER drive;
   CUTL_BUFFER directory;
   CUTL_BUFFER name;
...
void CUTL_PATH::AppendDirectory(LPCTSTR pSubDirectory) {
   CUTL_BUFFER drive;
   CUTL_BUFFER directory;
...
void CUTL_PATH::UpDirectory(CUTL_BUFFER* pLastDirectory) {
   ...
   if (pLastDirectory != NULL) {
      *pLastDirectory = LPCTSTR(directory) + delimiter;
      StripLeadingBackslash(*pLastDirectory);
   }
   if (delimiter >= 0)
      directory[delimiter] = _T('\0');
   SetDirectory(directory);
}

void CUTL_PATH::SetComponents (LPCTSTR drive, LPCTSTR directory, LPCTSTR name, LPCTSTR extension) {
   m_path.Realloc(_MAX_PATH);
   _tmakepath(m_path, drive, directory, name, extension);
}
...
void CUTL_PATH::CurrentDirectory() {
   ...
   _tgetcwd(driveDirectory, _MAX_PATH);
   Clear();
...
void CUTL_PATH::GetDriveDirectory(CUTL_BUFFER &driveDirectory) const {
	...
	if (drive.Len()) 
		driveDirectory += directory.InsCar(PATH_DRIVE_DELIMITER, 0);
}
...
void CUTL_PATH::GetComponents(CUTL_BUFFER *pDrive, CUTL_BUFFER *pDir, CUTL_BUFFER *pName, CUTL_BUFFER *pExt) const {
   ...
   _tsplitpath(m_path, 
      (pDrive ? LPTSTR(*pDrive) : NULL),
      (pDir   ? LPTSTR(*pDir)   : NULL),
      (pName  ? LPTSTR(*pName)  : NULL),
      (pExt   ? LPTSTR(*pExt)   : NULL)
   );
...
BOOL CUTL_PATH::GetFullyQualified(CUTL_BUFFER& fullyQualified) const {
   ...
   return _tfullpath(fullyQualified, m_path.GetSafe(), _MAX_PATH +1) ? TRUE : FALSE;
}
...
BOOL CUTL_PATH::RemoveDirectory() {
   ...
   if (m_path[0] == _T('\\') && m_path[1] == _T('\\')) // Es un UNC
      driveDirectory = m_path.GetSafe();
   else
      GetDriveDirectory(driveDirectory);
	if ((driveDirectory[driveDirectory.Len() - 1] == _T('\\') || driveDirectory[driveDirectory.Len() - 1] == '/')) driveDirectory[driveDirectory.Len() - 1] = _T('\0');
...
BOOL CUTL_PATH::ChangeDirectory() {
   CUTL_BUFFER driveDirectory;
   GetDriveDirectory(driveDirectory);
	return (_tchdir(driveDirectory) == 0);
}
...
BOOL CUTL_PATH::Delete(BOOL evenIfReadOnly) {
   ...
   //off
   //if(evenIfReadOnly && _waccess((const wchar_t*)(LPCTSTR)m_path, _S_IWRITE) != -1) { //does the file have write access
   if(evenIfReadOnly) { //does the file have write access
      if(_tchmod(m_path, _S_IWRITE) == -1) 
         return FALSE;
   }
   return !_tremove(m_path);
}

BOOL CUTL_PATH::Rename(LPCTSTR newPath) {
   return !_trename(m_path, newPath);
}
...
CUTL_PATH& CUTL_PATH::operator= (LPCTSTR path) {
   m_path = path;
   return *this;
}

CUTL_PATH::operator LPCTSTR() const {
...
CCR3_DATE::CCR3_DATE(LPCTSTR date) {
   if (!UTL_strlen(date)) {
      Clear();
   }
   else {
      _CRCHECK(UTL_strlen(date) == 17);
      UTL_strcpy(m_info, _T("00000000000000000"));
      UTL_strncpy(m_info, date, UTL_strlen(date));
      CUTL_BUFFER buf(5);
      UTL_strncpy(buf, m_info,    4); buf[4] = _T('\0'); m_yy = UTL_atoi(buf);
      UTL_strncpy(buf, m_info+4,  2); buf[2] = _T('\0'); m_mm = UTL_atoi(buf);
      UTL_strncpy(buf, m_info+6,  2); buf[2] = _T('\0'); m_dd = UTL_atoi(buf);
      UTL_strncpy(buf, m_info+8,  2); buf[2] = _T('\0'); m_hh = UTL_atoi(buf);
      UTL_strncpy(buf, m_info+10, 2); buf[2] = _T('\0'); m_mi = UTL_atoi(buf);
      UTL_strncpy(buf, m_info+12, 2); buf[2] = _T('\0'); m_ss = UTL_atoi(buf);
      UTL_strncpy(buf, m_info+14, 3); buf[3] = _T('\0'); m_fs = UTL_atoi(buf);
   }                                            
}
...
CCR3_DATE &CCR3_DATE::operator=(LPCTSTR date) {
   if (!UTL_strlen(date)) {
      UTL_strcpy(m_info, _T("00000000000000000"));
...
CCR3_DATE &CCR3_DATE::operator=(time_t timeExpr) {
    ...
    target.Sf(_T("%04d%02d%02d%02d%02d%02d%02d000"),  // según la estructura TIMEREG 
...
void CCR3_DATE::SysDate() {
   ...
   ::GetLocalTime(&st);
    target.Sf(_T("%04d%02d%02d%02d%02d%02d%03d"),  /* según la estructura TIMEREG */
              m_yy = st.wYear, 
              m_mm = st.wMonth, 
              m_dd = st.wDay,
              m_hh = st.wHour,
...
BOOL CCR3_DATE::SetDate(int yy, int mm, int dd) {
   ...
   _stprintf(m_info, _T("%04d%02d%02d%s"), yy, mm, dd, m_info+8);
   m_yy = yy;
   m_mm = mm;
   m_dd = dd;
   return TRUE;
...
BOOL CCR3_DATE::SetTime(int hh, int mi, int ss, int fs) {
   ...
   _stprintf(m_info+8, _T("%02d%02d%02d%03d"), 
      m_hh = hh, 
      m_mi = mi, 
      m_ss = ss, 
      m_fs = fs
...
void CCR3_DATE::ResetDate() {
   UTL_strncpy(m_info, _T("00000000"), 8);
   m_yy = m_mm = m_dd = 0;
}

void CCR3_DATE::ResetTime() {
   UTL_strncpy(m_info+8, _T("000000000"), 9);
...
void CCR3_DATE::Format(formatStyle style, CUTL_BUFFER* target) {
   ...
   switch (style) {
      case DDMMYY:
         target->Sf(_T("%c%c/%c%c/%c%c"), m_info[6],m_info[7],m_info[4],m_info[5],m_info[2],m_info[3]);
         break;
      case DDHHMI:
         target->Sf(_T("%c%c %c%c:%c%c"),m_info[6], m_info[7], m_info[8], m_info[9], m_info[10], m_info[11]);
         break;       
      case DDMMYYHHMISS:
         target->Sf(_T("%c%c/%c%c/%c%c %c%c:%c%c:%c%c"),          
            m_info[6],  m_info[7],
            m_info[4],  m_info[5],
            m_info[2],  m_info[3],
            m_info[8],  m_info[9],
         ...
         break;               
      case DDMMYYHHMISSML:
         target->Sf(_T("%c%c/%c%c/%c%c %c%c:%c%c:%c%c,%c%c%c"),           
            m_info[6],  m_info[7],
            m_info[4],  m_info[5],
            m_info[2],  m_info[3],
            m_info[8],  m_info[9],
         ...
         break;   
      case HHMISS:
         target->Sf(_T("%c%c:%c%c:%c%c"), m_info[8], m_info[9], m_info[10], m_info[11], m_info[12], m_info[13]);
         break;
      case HHMI:
         target->Sf(_T("%c%c:%c%c"),m_info[8], m_info[9], m_info[10], m_info[11]);
         break;       
      default:
         _CRCHECK(FALSE);
   }
...
CUTL_PARSE::CUTL_PARSE(LPCTSTR srcStream, LPCTSTR charSet, TCHAR escape) { 
...
void CUTL_PARSE::SetCharSet(LPCTSTR charSet, TCHAR escape) {
   m_escape = escape;
   m_charSet.Sf(_T("%s%c"), UTL_Null(charSet), escape);
}

void CUTL_PARSE::SetStream(LPCTSTR srcStream) {
   m_stream = srcStream;
   m_offset = 0;   
...
CUTL_PARSE::token CUTL_PARSE::NextToken() {
   ...
   do {
      ...
      if (m_stream[m_offset] == m_escape) {
         ...
         tmp.NCopy((LPTSTR)m_stream +m_offset+1, i - m_offset-1);
         m_tokenArgs += (LPCTSTR) tmp;
         m_offset = i+1;
         //Cuenta y marca los parámetros
         m_tokenNumArgs = 1 + m_tokenArgs.RepCar(',', _T('\0'));
...
CUTL_PARSE::token CUTL_PARSE::LookAhead() {
   ...
   if (UTL_strcspn((LPTSTR)m_stream +m_offset, m_charSet) > 0) return T_STRING;
...
LPTSTR CUTL_PARSE::StrArg(UINT pos) {
   if (!pos || pos > m_tokenNumArgs) return NULL;
   int i = 0;
   while (--pos) i += UTL_strlen((LPTSTR) m_tokenArgs + i) +1; 
   return (LPTSTR)m_tokenArgs +i;
}                                                                                       
...
DWORD CUTL_PARSE::LongHexArg(UINT pos) {
   ...
   int i = 0;
   while (--pos) i += UTL_strlen((LPTSTR) m_tokenArgs + i) +1; 
   return UTL_strtoul((LPTSTR)m_tokenArgs +i, NULL, 16);
}
...
long CUTL_PARSE::LongArg(UINT pos) {
   ...
   int i = 0;
   while (--pos) i += UTL_strlen((LPTSTR) m_tokenArgs + i) +1; 
   return UTL_atol((LPTSTR)m_tokenArgs +i);
}
...
int CUTL_PARSE::IntArg(UINT pos) {
   ...
   int i = 0;
   while (--pos) i += UTL_strlen((LPTSTR) m_tokenArgs + i) +1; 
   return UTL_atoi((LPTSTR)m_tokenArgs +i);
}

TCHAR CUTL_PARSE::ChrArg(UINT pos) {
   if (!pos || pos > m_tokenNumArgs) return _T('\0');
   int i = 0;
   while (--pos) i += UTL_strlen((LPTSTR) m_tokenArgs + i) +1; 
...
void StripTrailingChar(CUTL_BUFFER& text, TCHAR traillingCh) {
	...
	if (!len) return;
	if (text[len -1] == traillingCh) text[len -1] = _T('\0');
}
...
int UTL_strncmp(const TCHAR *string1, const TCHAR *string2, size_t count ) {
   ...
   LPTSTR a1,a2;
   int result;
   _CRVERIFY(a1 = (LPTSTR) UTL_alloc(count +1, sizeof(TCHAR)));
   _CRVERIFY(a2 = (LPTSTR) UTL_alloc(count +1, sizeof(TCHAR)));
   UTL_strncpy(a1, string1, count); a1[count] = _T('\0');
   UTL_strncpy(a2, string2, count); a2[count] = _T('\0');
